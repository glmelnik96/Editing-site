...
- **Найдена настоящая ошибка в готовом коде:** `render.py::_subtitles_file` узнавал только `source == "transcript"` и на новом источнике падал бы `KeyError` — то есть вычитанные реплики в ролик не попали бы вовсе. Починено списком `BUILD_SOURCES`.
- **Тест из плана не сработал бы дословно:** `for field, _ in exc.value.errors` разбирает словари как пары и всегда даёт ложь. Заменён на существующий в файле хелпер.
- **Загруженный чужой транскрипт** не выравнивает времена слов внутри сегмента, и наложение слов дало бы наложение реплик — человек увидел бы `422` на документе, который сам не писал. При сборке начало реплики подтягивается к концу предыдущей.
- **Панели точек и сборки стали ленивыми:** монтируются при первом открытии вкладки, а не при входе в редактор — это два запроса к серверу, которых больше нет на каждом входе.
- **Живая проверка на стенде:** «Собрать субтитры» дала три реплики с разбиением по типографике, правка текста уехала на сервер (версия 3, источник `cues`), разрезание дало четыре реплики, Ctrl+Z вернул три. Реплика под курсором подсвечивается.
- **Счётчик реплик склоняется** («1 реплика», «3 реплики», «11 реплик») — вместе с разрезанием и подсказкой о негодной реплике это единственная чистая логика панели, она под тестами.
