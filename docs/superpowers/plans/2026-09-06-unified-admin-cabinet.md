...
- **Секрет соседа.** До начала работы починили причину `403` у VideoBoard: в `/opt/videoboard/.env` ключ назывался `SERVICE_TOKEN`, а сервис читает `BOARD_SERVICE_TOKEN`. Значение совпадало, имя нет. Переименовали, перезапустили `board-api`, оба соседа отвечают `200`.
- **Задача 2.** В присланных тестах было две настоящие ошибки: `make_settings(**kw)` с тем же ключом и явным аргументом давал «multiple values», а `request.url.path` у httpx приходит уже раскодированным — проверять экранирование почты нужно по `raw_path`. Проверка «404 при удалении» переехала с поиска подстроки в сообщении на аргумент `missing_ok` у `_request`. Добавлен срез завершающего слэша у адресов соседей: иначе путь склеивался в `//api/...`.
- **Задача 4.** `collect` теперь всегда добавляет строку администратора, даже когда его нет ни в одном списке: доступ у него из конфигурации, и строка просто исчезала бы с экрана. Нашлось при разборе живого ответа VideoBoard — его белый список пуст.
- **Живая проверка** шла на локальном стенде через ssh-туннели к настоящим соседям (`-L 8020` и `-L 8014`), а не на моках: домен ВМ с ПК не резолвится, а формы ответов хотелось увидеть настоящие.
- **Значок вкладки.** Заодно добавлена общая для трёх сервисов иконка `web/public/favicon.svg` (тёмная плитка с зелёной точкой) и ссылка с `?v=2` — раньше значка не было вовсе.
