...
- **Задача 3.** `_sources_for` получает владельца отдельным аргументом (`job["user_id"]`), а не из словаря проекта: ключа `user_id` там нет.
- **Задача 5.** `parse_file_url` вернулся четвёркой `(user_id, owner_id, name, kind)`; имя рендера обязано быть `{id}.mp4`. Приложение отдаёт ролик с `filename*=UTF-8''…` (RFC 5987): названия проектов кириллические, а в заголовок HTTP такое напрямую не положить. Caddy отдаёт голый `attachment`, имя ставит браузер по атрибуту `download` у ссылки — тот же origin.
- **Задача 6, по итогам ревью.** Опрос задания прекращается на неустранимой ошибке (401, 404): раньше он бился бы в дверь каждые две секунды вечно, а кнопки сборки оставались бы заблокированными. Предикат `isRetryable` переехал из `web/src/upload.ts` в `web/src/api.ts` и используется обоими.
- **Задача 6, по итогам ревью.** `createSaver.flush` теперь ждёт, пока очередь сохранений опустеет, и бросает ошибку, если последнее сохранение не записалось (сеть, 401, конфликт версий, отказ проверки). Без этого `await onBeforeStart()` возвращался до записи, и воркер мог собрать предыдущую версию документа.
- **Задача 7.** Смоук читает заголовки в нижнем регистре: `dict(resp.headers)` теряет регистронезависимость, и проверка вложения ложно падала на живом сервере.
