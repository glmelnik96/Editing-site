...
def renders(...):  # список готовых роликов проекта
```
...
def job(...)  # id, type, status, progress, error, created_at, finished_at


@router.post("/jobs/{job_id}/cancel", status_code=204)
def cancel(...)  # только свои задания; отменяет очередь и выполняющееся
```

Все выборки фильтруются по `user_id`, чужое даёт 404. Роутер подключить в `server/app/main.py`.
