...
def _hand_back_glue(take: list[str], index: int, total: int) -> int:
    """Т3: реплика не кончается служебным словом, пока дальше есть слова — оно уезжает в
    следующую. Не больше двух слов подряд: иначе цепочка «и в то» растащит половину фразы.
    ...
    for _ in range(2):
        if index >= total or len(take) < 2:
            break
        last = take[-1]
        if not is_glue_word(last) or _ENDS_PUNCT.search(last):
            break
        take.pop()
        index -= 1
    return index
