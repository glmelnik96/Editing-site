...
def handle_render(conn: sqlite3.Connection, settings: Settings, job: sqlite3.Row) -> None:
    ...
    with transaction(conn):
        # Сборку могли отменить, пока шло кодирование: воркер видит отмену только на очередном
        # пульсе, а последние секунды проходят уже без проверок — и отменённое задание всё равно
        # клало готовый ролик. Удаление проекта прикрыто внешним ключом: строки проекта нет,
        # и вставка падает сама.
        alive = conn.execute(
            "SELECT 1 FROM jobs WHERE id = ? AND status = 'running'", (job["id"],)
        ).fetchone()
