...
describe('сводка вкладки Рендер', () => {
  it('говорит человеческим языком, без argv', () => {
    ...
    expect(text).toContain('Дорожка A2: 1 звук.')
  ...
  it('без дакинга не обещает приглушение', () => {
    ...
    expect(text).toContain('Дорожка A2')
  ...
  it('называет переходы и дорожку A2, если они есть', () => {
    const withFade = {
      ...doc,
    ...
    expect(text).toContain('Дорожка A2: 2 звука.')
  })
})
